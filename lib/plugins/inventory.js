...
function inject(bot) {
  ...
  function openFurnace(furnaceBlock) {
    ...
    bot.client.on(0x31, onUpdateWindowProperty);
    furnace.once("close", onClose);
    return furnace;
    function onClose() {
      bot.client.removeListener(0x31, onUpdateWindowProperty);
    }
  ...
  bot.client.on(0x32, function(packet) {
    // confirm transaction
  ...
  bot.client.on(0x2d, function(packet) {
    // open window
  ...
  bot.client.on(0x2e, function(packet) {
    // close window
    var oldWindow = bot.currentWindow;
    bot.currentWindow = null;
    bot.emit("windowClose", oldWindow);
  });
  bot.client.on(0x2f, function(packet) {
    // set slot
  ...
  bot.client.on(0x30, function(packet) {
    // set window items
    var window = packet.windowId === 0 ? bot.inventory : bot.currentWindow;
    if (! window || window.id !== packet.windowId) return;
    var i, item;
