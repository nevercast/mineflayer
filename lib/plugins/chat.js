...
function inject(bot) {
  bot.client.on(0x02, function(packet) {
    ...
    function parseJsonMessage(jsonMessage) {
      ...
      if (typeof jsonMsg.translate === 'string' && jsonMsg.translate.match(/^chat\./)) {
        ...
        content = jsonMsg.with[1].replace(incomingFilter, '');
        bot.emit('chat', username, content, jsonMsg);
      } else if (jsonMsg.translate === "commands.message.display.incoming") {
        // whispered chat
        username = jsonMsg.with[0].text;
        content = jsonMsg.with[1].replace(incomingFilter, '');
        bot.emit('whisper', username, content, jsonMsg);
  ...
  function chatWithHeader(header, message) {
    ...
    message.split("\n").forEach(function(subMessage) {
      ...
      for (i = 0; i < subMessage.length; i += lengthLimit) {
        ...
        bot.client.write(0x01, {message: smallMsg});
      }
    });
  }
